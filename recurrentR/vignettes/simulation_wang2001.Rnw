...
<<setup>>=
library(knitr)
opts_chunk$set(cache=TRUE, echo=FALSE, cache.path='cache_simulation_wang2001/')
@

<<wang_2001_generation>>=
...
<<wang_2001_parametric_bootstrap>>=
...
for(i in seq_len(B)) {
  obj.log[[i]] <- gen_wang_2001()
  temp <- Wang2001(obj=obj.log[[i]])
  gamma.hat[i,] <- temp$gamma.bar.hat
  if (abs(gamma.hat[i,2]) > 18) browser()
...
<<wang_2001_nonparametric_bootstrap>>=
obj <- gen_wang_2001()
wang_2001 <- Wang2001(obj=obj, methods="bootstrap", B = 1000)
wang_2001.a <- Wang2001(obj=obj, methods="asymptotic")
curve(Lambda_0, 0, 10, col = 2, ylim = c(0, 1.2), main = "Simulation of Lambda")
...
plotCI(1:2 + 0.1, wang_2001.a$gamma.bar.hat, uiw=qnorm(0.975) * sqrt(diag(wang_2001.a$gamma.bar.hat.var)), col = 4, main = "Simulation of gamma", xlab = "", ylab = "", xaxt="n", xlim=c(1,2.1), , ylim = c(-1, 2))
plotCI(1:2, apply(gamma.hat, 2, mean), uiw=qnorm(0.975) * apply(gamma.hat, 2, sd), add = TRUE)
