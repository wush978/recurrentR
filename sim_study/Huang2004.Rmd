...
plotCI(1:2, apply(gamma.hat, 2, mean), uiw=apply(gamma.hat, 2, sd), 
       main = "Parametric Bootstrap", xlab = "", ylab = "gamma.bar.hat", xaxt="n")
...
lines(x.eval, h.eval - qnorm(0.975) * h.eval.sd, lty = 2)
plotCI(1, mean(alpha.hat), uiw = sd(alpha.hat), main = "Parametric Bootstrap", ylab="alpha.hat", xlab="n", xaxt="n")
...
Lambda_0.hat.upper <- Vectorize(function(x) huang_2004$Lambda_0.hat(x) + 2 * sqrt(huang_2004$Lambda_0.hat.var(x)))
curve(Lambda_0.hat.upper, add=TRUE, col=3, 0, 10, lty = 2)
Lambda_0.hat.lower <- Vectorize(function(x) huang_2004$Lambda_0.hat(x) - 2 * sqrt(huang_2004$Lambda_0.hat.var(x)))
